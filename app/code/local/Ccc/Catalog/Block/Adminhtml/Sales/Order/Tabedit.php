<?php
class Ccc_Catalog_Block_Adminhtml_Sales_Order_Tabedit extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        if ($this->canShowAddressValidationButton()) {
            $confirmationMessage = $this->jsQuoteEscape($this->getAddressValidationConfirmationMessage());
            $this->addButton('send_addressvalidation', array(
                'label' => Mage::helper('sales')->__('Address Validate'),
                'onclick' => "addressValidate('{$order->getId()}', '{$this->getUrl('*/sales_order/validateAddress')}')",
            )
            );
        }
        ...
        if (
            $this->_isAllowedAction('ship') && $order->canShip()
    ...
    public function getAddressValidateUrl()
    {
        return $this->getUrl('*/*/addressValidate');
    }

    public function canShowAddressValidationButton()
    {
        $order = $this->getOrder();
        return ($order->getAddressValidationRequired() == '1');
    }

    public function getAddressValidationConfirmationMessage()
    {
        return Mage::helper('sales')->__('Are you sure you want to validate the address?');
    }
