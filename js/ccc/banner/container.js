...
varienGrid.prototype = {
  ...
  bindFieldsChange: function () {
    ...
    var dataElements = $(this.containerId + this.tableSufix)
      .down("tbody")
      .select("input", "select");
    for (var i = 0; i < dataElements.length; i++) {
      Event.observe(
        dataElements[i],
        "change",
        dataElements[i].setHasChanges.bind(dataElements[i])
      );
    }
  },
  ...
  doFilter: function () {
    ...
    filters.push(bannerGrid_status);
    var elements = [];
...
function openGridRow(grid, event) {
  var element = Event.findElement(event, "tr");
  if (
    ["a", "input", "select", "option"].indexOf(
      Event.element(event).tagName.toLowerCase()
    ) != -1
  ) {
    return;
  }
...
varienGridMassaction.prototype = {
  ...
  initMassactionElements: function () {
    ...
    this.select.observe(
      "change",
      this.onSelectChange.bindAsEventListener(this)
    );
  ...
  getOldCallback: function (callbackName) {
    return this.oldCallbacks[callbackName]
      ? this.oldCallbacks[callbackName]
  ...
  onSelectChange: function (evt) {
    ...
    if (item) {
      this.formAdditional.update(
        $(this.containerId + "-item-" + item.id + "-block").innerHTML
      );
    } else {
      this.formAdditional.update("");
    }
  ...
  checkCheckboxes: function () {
    this.getCheckboxes().each(
      function (checkbox) {
        checkbox.checked = varienStringArray.has(
          checkbox.value,
          this.checkedString
        );
      }.bind(this)
  ...
  onMassactionComplete: function (transport) {
    if (this.currentItem.complete) {
      try {
        var listener =
          this.getListener(this.currentItem.complete) ||
          Prototype.emptyFunction;
...
serializerController.prototype = {
  ...
  initialize: function (
    ...
    this.grid.reloadParams = {};
    this.grid.reloadParams[this.reloadParamName + "[]"] =
  ...
  inputChange: function (event) {
    ...
    if (element && element.checkboxElement && element.checkboxElement.checked) {
      this.gridData.get(element.checkboxElement.value)[element.name] =
        element.value;
      this.hiddenDataHolder.value = this.serializeObject();
    }
  },
    ...
    if (this.multidimensionalMode) {
      ...
      if (checkbox && inputs.length > 0) {
        ...
        for (var i = 0; i < inputs.length; i++) {
          ...
          if (
            this.gridData.get(checkbox.value) &&
            this.gridData.get(checkbox.value)[inputs[i].name]
          ) {
  ...
  getGridDataHash: function (_object) {
    return $H(
      this.multidimensionalMode ? _object : this.convertArrayToObject(_object)
    );
  },
  getDataForReloadParam: function () {
    return this.multidimensionalMode
      ? this.gridData.keys()
